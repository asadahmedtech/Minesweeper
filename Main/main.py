...
class Tile_Create(object):
    ...
    def __NeighbourMap(self):
        ...
        for i in range(self.row):
            ...
            for j in range(self.column):
                Temp.append('M' if self.__isMine(
                    (i, j)) else self.__NeighbourMines((i, j)))
    ...
    def NewMapGenerator(self):
        self.__TempArray = [['0'for j in range(self.column)]
                            for i in range(self.row)]
...
class Tile_Actions(Tile_Create):
    ...
    def __init__(self, Row, Column, NOFM):
        super(Tile_Actions, self).__init__(Row, Column, NOFM)
              # it will take the total mines and remove every mine that is
              # flagged
        self.Armed = self.no_of_mines
        self.MineOpened = False
        self.FlaggedMap = [['0' for i in range(self.column)]
                           for j in range(self.row)]
        self.OpenMap = [[''for i in range(self.column)]
                        for j in range(self.row)]
        self.FlaggedTile = NOFM
        # self.__PrintArray(self.NeighbourMap,self.row,self.column)
        self.VisitedMap = [['' for i in range(self.column)]
                           for j in range(self.row)]
...
class GameBoard:
    ...
    def __init__(self):
        ...
        self.images = {
            ...
            'uncovered_0': pygame.transform.scale(
                pygame.image.load('uncovered_tile_0.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_1': pygame.transform.scale(
                pygame.image.load('uncovered_tile_1.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_2': pygame.transform.scale(
                pygame.image.load('uncovered_tile_2.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_3': pygame.transform.scale(
                pygame.image.load('uncovered_tile_3.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_4': pygame.transform.scale(
                pygame.image.load('uncovered_tile_4.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_5': pygame.transform.scale(
                pygame.image.load('uncovered_tile_5.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_6': pygame.transform.scale(
                pygame.image.load('uncovered_tile_6.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_7': pygame.transform.scale(
                pygame.image.load('uncovered_tile_7.png'),
                (self.blockSize,
                 self.blockSize)),
            'uncovered_8': pygame.transform.scale(
                pygame.image.load('uncovered_tile_8.png'),
                (self.blockSize,
                 self.blockSize)),
            'covered_mine': pygame.transform.scale(
                pygame.image.load('covered_mine.png'),
                (self.blockSize,
                 self.blockSize)),
            'flagged': pygame.transform.scale(
                pygame.image.load('flagged_tile.png'),
                (self.blockSize,
                 self.blockSize)),
            'correct_mine': pygame.transform.scale(
                pygame.image.load('correct_tile.png'),
                (self.blockSize,
                 self.blockSize)),
            'mined': pygame.transform.scale(
                pygame.image.load('mined_tile.png'),
                (self.blockSize,
                 self.blockSize)),
            'openedmine': pygame.transform.scale(
                pygame.image.load('selected_mined_tile.png'),
                (self.blockSize,
                 self.blockSize)),
            ...
            'restart': pygame.transform.scale(
                pygame.image.load('restart.png'),
                (self.blockSize,
                 self.blockSize)),
            'game_over': pygame.transform.scale(
                pygame.image.load('game_over.jpg'),
                (self.blockSize,
                 self.blockSize)),
            'game_won': pygame.transform.scale(
                pygame.image.load('game_won.jpg'),
                (self.blockSize,
                 self.blockSize)),
            'mine-home': pygame.transform.scale(
                pygame.image.load('Mine_Homepage.png'), (200, 200)),
        ...
        for i in range(self.row):
            for j in range(self.column):
                if self.MineObject.OpenMap[i][j] == 'O':
                    Temp_Image = self.images[
                        'uncovered_' + str(self.MineObject.NeighbourMap[i][j])]
    ...
    def __button(
     ...
     stat=None):
        ...
        if ActiveColor is not None:
            ...
            pygame.draw.rect(
                self.gameDisplay if stat is None else self.highscoreDisplay,
                buttoncolor,
                (buttonx,
                 buttony,
                 width,
    ...
    def homePage(self):
        ...
        while self.home:
            ...
            #
            #
            textSurface1 = pygame.font.SysFont(
                "comicsansms",
                20).render('- MADE BY ASAD AHMED',
                           True,
                           (242,
                            255,
                            0))
            ...
            self.__button('EASY', 50, 300, 100, 50, (
                34, 177, 76), (247, 247, 247), (0, 255, 0), 'easy')
            self.__button('MEDIUM', 250, 300, 100, 50, (
                200, 200, 0), (247, 247, 247), (255, 255, 0), 'medium')
            self.__button('HARD', 450, 300, 100, 50, (
                200, 0, 0), (247, 247, 247), (255, 0, 0), 'hard')
            self.__button('HIGH SCORE', 400, 200, 180, 50, (
                28, 18, 142), (247, 247, 247), (98, 84, 255), 'highscore')
    ...
    def highscorePage(self):
        ...
        while self.highscore:
            ...
            textSurface1 = pygame.font.SysFont(
                "comicsansms", 50).render('HIGH', True, (0, 255, 51))
            textRect1 = textSurface1.get_rect()
            textRect1.center = 100, 150
            self.highscoreDisplay.blit(textSurface1, textRect1)
            textSurface2 = pygame.font.SysFont(
                "comicsansms", 50).render('SCORE', True, (0, 255, 51))
            textRect2 = textSurface2.get_rect()
            textRect2.center = 100, 250
            self.highscoreDisplay.blit(textSurface2, textRect2)
            self.__button('EASY : ' + str((self.HighScore['easy'])[0]) + ':' + str(
                (self.HighScore['easy'])[1]), 200, 80, 400, 100, (35, 35, 35), (0, 255, 4), stat='high')
            self.__button('MEDIUM : ' + str((self.HighScore['medium'])[0]) + ':' + str(
                (self.HighScore['medium'])[1]), 200, 180, 400, 100, (35, 35, 35), (0, 255, 4), stat='high')
            self.__button('HARD : ' + str((self.HighScore['hard'])[0]) + ':' + str(
                (self.HighScore['hard'])[1]), 200, 280, 400, 100, (35, 35, 35), (0, 255, 4), stat='high')
            cur = pygame.mouse.get_pos()
            click = pygame.mouse.get_pressed()
            homeX, homeY = 0, 10
                    ...
                    if not self.cheatUsed:
                        if self.CurrentScore[0] < (self.HighScore[self.difficulty])[0]:
                            ...
                            if self.CurrentScore[1] < (self.HighScore[self.difficulty])[1]:
                                self.HighScore[
                                    self.difficulty] = self.CurrentScore
                                self.__storeHighScore()
                ...
                if self.TimeDisplay:
                    ...
                    self.__button(
                        ...
                        (247,
                         39,
                         39))
                self.__button('0' + str(self.MineObject.FlaggedTile) if 10 > self.MineObject.FlaggedTile >= 0 else str(
                    self.MineObject.FlaggedTile), self.gameDimens[0] - 80, 10, 80, 30, (242, 242, 242), (247, 39, 39))

                # FOR RESTART BUTTON
                cur = pygame.mouse.get_pos()
                click = pygame.mouse.get_pressed()
                buttonX, buttonY = (int(int(self.gameDimens[0]) / 2 - int(self.blockSize) / 2)), (
                    int(self.Top_bar) / 2 - int(self.blockSize) / 2)
                if buttonX + self.blockSize > cur[0] > buttonX and buttonY + self.blockSize > cur[1] > buttonY and click[0] == 1:
                    self.MineObject = Tile_Actions(
                        self.row,
                        self.column,
                        self.no_of_mines)
                    self.startTime = 0
                    ...
                    self.cheatUsed = False
                self.gameDisplay.blit(
                    self.images['restart'], (buttonX, buttonY))
